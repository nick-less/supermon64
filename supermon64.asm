...
; -----------------------------------------------------------------------------
; variables
ACMD:   .byte 1             ; addressing command
LENGTH: .byte 1             ; length of operand
MNEMW:  .RES 3             ; 3 letter mnemonic buffer
SAVX:   .byte 1             ; 1 byte temp storage, often to save X register
OPCODE: .byte 1             ; current opcode for assembler/disassembler
UPFLG:  .byte 1             ; flag: count up (bit 7 clear) or down (bit 7 set)
DIGCNT: .byte 1             ; digit count
INDIG:  .byte 1             ; numeric value of single digit
NUMBIT: .byte 1             ; numeric base of input
STASH:  .byte 2             ; 2-byte temp storage
...
PCH:    .byte 1             ; program counter high byte
PCL:    .byte 1             ; program counter low byte
SR:     .byte 1             ; status register
ACC:    .byte 1             ; accumulator
XR:     .byte 1             ; X register
YR:     .byte 1             ; Y register
SP:     .byte 1             ; stack pointer

STORE:  .byte 2             ; 2-byte temp storage
CHRPNT: .byte 1             ; current position in input buffer
SAVY:   .byte 1             ; temp storage, often to save Y register
U9F:    .byte 1             ; index into assembler work buffer
...
MSG8:   .ASCIIZ "  "              ; pad non-existent byte: skip 3 spaces
        .BYTE $20+$80
